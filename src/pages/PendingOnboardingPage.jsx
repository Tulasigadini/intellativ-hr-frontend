...
const FLOW_STEPS   = ['Personal Info','Work History','Job Details','Documents','Insurance','Complete'];

/* ─── Step config — icon + label for each onboarding step ───────────────────── */
const STEP_CONFIG = [
...
/* ─── Employee Card ──────────────────────────────────────────────────────────── */
const EmpCard = ({ emp, onResume, onSendEmail }) => {
      ...
      <div style={{ display:'flex', alignItems:'center', gap:16, flexWrap:'wrap' }}>
        ...
        <div style={{ display:'flex', flexDirection:'column', alignItems:'flex-end', gap:6, flexShrink:0 }}>
          ...
          <button
            ...
            style={{
              background: c.percent === 100 ? '#22c55e' : '#0d5c7a',
          ...
          {/* Send joining details email — only if employee has personal email + has incomplete docs/insurance */}
          {emp.personal_email && (!c.has_documents || !c.has_insurance) && (
            <button
              onClick={e => { e.stopPropagation(); onSendEmail(emp); }}
              title={`Send document/insurance link to ${emp.personal_email}`}
              style={{
...
function DocViewer({ doc, employeeId }) {
  ...
  React.useEffect(() => {
    ...
    fetch(`${process.env.REACT_APP_API_URL || 'https://intellative-hr-backend.onrender.com/api/v1'}/employees/${employeeId}/documents/${doc.id}/download`, {
...
export default function PendingOnboardingPage() {
  ...
  const openResume = async (emp) => {
    ...
    if (c.has_personal_info) startStep = 2; // work history always skippable
  ...
  const sendJoiningEmail = async (emp) => {
    if (!window.confirm(`Send joining details email to ${emp.personal_email}?\n\nThis will include their login credentials and a link to upload documents and fill insurance details.`)) return;
    try {
  ...
  const saveInsurance = async () => {
    ...
    await btnRun('insurance', async () => {
      await onboardingAPI.saveInsurance(selected.id, insuranceForm);
  ...
  /* ── Activate (only for pending employees) ── */
      ...
      <div style={{ maxWidth:860, margin:'0 auto' }}>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:14, marginBottom:20 }}>
          <button onClick={closeFlow}
            style={{ background:'#f3f4f6', border:'none', borderRadius:10, padding:'8px 14px', fontSize:13, fontWeight:700, color:'#374151', cursor:'pointer' }}>
            ← Back
          </button>
          <div style={{ flex:1 }}>
          ...
          <div style={{ display:'flex', gap:6 }}>
            {STEP_CONFIG.map(s => (
              <div key={s.key} title={s.label} style={{
                ...
                background: c[s.key] ? '#f0fdf4' : '#fff7ed',
                border: `1.5px solid ${c[s.key] ? '#86efac' : '#fed7aa'}`,
        ...
        {/* Incomplete steps banner */}
        {c.missing_steps?.length > 0 && (
          <div style={{ padding:'12px 16px', background:'#fff7ed', border:'1px solid #fed7aa',
            borderRadius:10, marginBottom:16, fontSize:13, color:'#92400e', display:'flex', gap:8, alignItems:'center' }}>
            <span style={{ fontSize:16 }}>⚠</span>
            <span><strong>Incomplete steps:</strong> {c.missing_steps.join(' · ')} — click each tab above to complete.</span>
          </div>
        )}

        {/* Step tabs */}
        <div style={{ display:'flex', gap:2, background:'#f3f4f6', padding:4, borderRadius:12,
          border:'1px solid #e5e7eb', marginBottom:20, overflowX:'auto' }}>
          {FLOW_STEPS.map((label, i) => {
            ...
            return (
              <button key={i} onClick={() => setStep(i)} style={{
                ...
                color: active ? '#111827' : stepDone ? '#15803d' : '#9ca3af',
        ...
        <div className="card" style={{ maxWidth:820, margin:'0 auto' }}>
          ...
          {step === 0 && (
            <div>
              ...
              <p style={{ fontSize:13, color:'var(--text-secondary)', marginBottom:20 }}>Update employee personal details.</p>
            ...
            <div>
              ...
              {serverDocs.length > 0 && (
                <div style={{ padding:'10px 14px', background:'#f0fdf4', border:'1px solid #86efac', borderRadius:8, marginBottom:16, fontSize:13, color:'#15803d', fontWeight:600 }}>
                  ✓ {serverDocs.length} document(s) already uploaded. You can upload new ones to replace them.
                </div>
              )}
                    ...
                    {cat.docs.map(dt => {
                      ...
                      return (
                        <div key={dt.value} style={{
                          border: `2px solid ${hasPending ? '#f59e0b' : hasServer ? '#16a34a' : dt.required ? 'var(--accent)' : 'var(--border)'}`,
                          borderRadius:10, padding:12, background: hasPending ? '#fffbeb' : hasServer ? '#f0fdf4' : 'var(--bg)',
                          ...
                          <div style={{ display:'flex', alignItems:'center', gap:8 }}>
                            ...
                            <div style={{ flex:1 }}>
                              ...
                              {hasServer && !hasPending && (
                                <div style={{ fontSize:11, color: isVerified ? '#16a34a' : '#f59e0b', fontWeight:600 }}>
                                  {isVerified ? '✓ Verified' : '⏳ Pending verification'}
                                </div>
                              )}
                              {hasPending && (
                                <div style={{ fontSize:11, color:'#f59e0b', fontWeight:600 }}>🔄 Ready to upload (will replace)</div>
                              )}
                              {!hasServer && !hasPending && (
                                <div style={{ fontSize:11, color:'var(--text-muted)' }}>Not uploaded</div>
                              )}
                            </div>
                          </div>
                          {hasServer && !hasPending && (
            ...
            <div>
              ...
              {insuranceSaved || c.has_insurance ? (
                <div style={{ textAlign:'center', padding:'32px 20px' }}>
                  ...
                  <p style={{ color:'var(--text-secondary)', marginTop:8 }}>Tasks have been created for the Insurance & HR teams.</p>
                ...
                <>
                  ...
                  <div style={{ display:'flex', justifyContent:'space-between', marginTop:16, flexWrap:'wrap', gap:10 }}>
                    ...
                    <div style={{ display:'flex', gap:10, flexWrap:'wrap' }}>
                      <button className="btn btn-ghost" onClick={() => { setInsuranceSkipped(true); setStep(5); }}>Skip for now →</button>
    ...
    <div style={{ maxWidth:1000, margin:'0 auto' }}>
        ...
        <div style={{ display:'flex', gap:10, marginBottom:20, flexWrap:'wrap' }}>
          <button
            ...
            style={{ padding:'8px 16px', borderRadius:10, border:'none', cursor:'pointer', fontWeight:700, fontSize:13,
              background: filterStatus === 'all' ? '#0d5c7a' : '#f3f4f6',
          ...
          <button
            ...
            style={{ padding:'8px 16px', borderRadius:10, border:'none', cursor:'pointer', fontWeight:700, fontSize:13,
              background: filterStatus === 'pending' ? '#92400e' : '#fffbeb',
          ...
          <button
            ...
            style={{ padding:'8px 16px', borderRadius:10, border:'none', cursor:'pointer', fontWeight:700, fontSize:13,
              background: filterStatus === 'active' ? '#1d4ed8' : '#eff6ff',
              color: filterStatus === 'active' ? '#fff' : '#1d4ed8',
      ...
      <div style={{ marginBottom:18 }}>
        <input
          ...
          style={{ width:'100%', maxWidth:420, padding:'9px 14px', borderRadius:10, border:'1.5px solid #e5e7eb', fontSize:13, outline:'none' }}
        />
      </div>
      ...
      {listLoading ? (
        ...
        <div style={{ textAlign:'center', padding:'60px 20px', background:'#fff', borderRadius:16, border:'1px solid #e5e7eb' }}>
          ...
          <p style={{ fontSize:13, color:'#9ca3af', marginTop:6 }}>
            {list.length === 0
              ? 'Every employee has completed all onboarding steps.'
              : 'Try a different search or filter.'}
          </p>
          {list.length === 0 && (
            <button className="btn btn-primary" style={{ marginTop:20 }} onClick={() => navigate('/onboarding')}>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
          {filtered.map(emp => (
            <EmpCard key={emp.id} emp={emp} onResume={openResume} onSendEmail={sendJoiningEmail} />
          ))}
        </div>
      )}
    </div>
