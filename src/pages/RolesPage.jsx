...
function buildTree(roles, parentId = null) {
  return roles
    ...
    .map(r => ({ 
      ...r, 
      sub_roles: buildTree(roles, r.id) 
    }));
}

function RoleNode({ node }) {
  const colors = [
    '#0d5c7a', '#1a8cad', '#29b6e0', '#0ea5e9', 
...
export default function RolesPage() {
  ...
  const del = async (id) => {
    ...
    try {
      ...
      toast.error('Cannot delete — linked to employees');
    }
  };

  const filteredRoles = filterDept 
    ? allRoles.filter(r => r.department_id === filterDept) 
    : allRoles;
    ...
    <>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
          ...
          {treeByDept.length === 0 && (
            <div className="empty-state col-span-full py-16">
                ...
                {filteredRoles.map(r => (
                  <tr key={r.id}>
                    ...
                    <td style={{ color: 'var(--text-muted)' }}>
                      {r.parent_role_id 
                        ? allRoles.find(x => x.id === r.parent_role_id)?.name || '—' 
                        : '—'}
                    </td>
                    <td>
          ...
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="modal-footer">
              ...
              <LoadingButton 
                ...
                loading={saving} 
                onClick={save}
                disabled={!form.name || !form.code || !form.department_id}
              >
                Create Role
              </LoadingButton>
            </div>
